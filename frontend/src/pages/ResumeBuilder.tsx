...
type ResumeFormValue = string | string[];
type ApiErrorLike = {
  response?: {
    data?: {
      message?: string;
...
const ResumeBuilder = () => {
  ...
  const handleInputChange = (section: keyof ResumeInputData, field: string, value: ResumeFormValue, index?: number) => {
    setUserInfo(prev => {
      const newState = structuredClone(prev);

      if (index !== undefined && Array.isArray(newState[section])) {
        if (!newState[section][index]) newState[section][index] = {};
        (newState[section][index] as Record<string, unknown>)[field] = value;
      } else if (typeof newState[section] === 'object' && newState[section] !== null) {
        (newState[section] as Record<string, unknown>)[field] = value;
      } else {
  ...
  const addResponsibility = (expIndex: number) => {
    const respInput = respInputs[expIndex] || "";
  ...
  const removeCertificationBlock = (index: number) => {
    setUserInfo(prev => ({
      ...prev,
              ...
              <CardContent className="p-0">
                <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
                          ...
                          <div className="space-y-3">
                            ...
                            <div className="flex gap-2">
                              <Input
                                ...
                                value={respInputs[index] || ""}
                                onChange={(e) => setRespInputs(prev => ({ ...prev, [index]: e.target.value }))}
                                ...
                                onKeyDown={(e) => {
                                  if (e.key === 'Enter' && (respInputs[index] || "").trim()) {
                                    e.preventDefault();
                  ...
                  <TabsContent value="projects" className="p-6 space-y-6">
                    {(userInfo.projects && userInfo.projects.length > 0 ? userInfo.projects : [{ name: "", description: "", technologies: [], link: "" }]).map((project, index) => (
                      <Card key={index} className="p-0 border border-gray-200 shadow-sm relative overflow-hidden">
                        <div className="absolute top-0 left-0 w-1 h-full bg-blue-500"></div>
                        <CardHeader className="p-4 bg-gray-50">
                          <div className="flex justify-between items-center">
                            <CardTitle className="text-base font-medium flex items-center">
                        ...
                        <CardContent className="p-4 space-y-4">
                          <div className="space-y-3">
                            <Label htmlFor={`project-name-${index}`} className="text-gray-700">Project Name</Label>
                            <Input
                          ...
                          <div className="space-y-3">
                            <Label htmlFor={`project-description-${index}`} className="text-gray-700">Project Description</Label>
                            <Textarea
                              id={`project-description-${index}`}
                              value={project.description || ''}
                              onChange={(e) => handleInputChange('projects', 'description', e.target.value, index)}
                              placeholder="Developed a full-stack e-commerce platform with authentication, cart, and payment features."
                              rows={3}
                              className="border-gray-300 focus:border-blue-400"
                            />
                          </div>
                          <div className="space-y-3">
                            <Label htmlFor={`project-technologies-${index}`} className="text-gray-700">Technologies Used</Label>
                            <Textarea
                          ...
                          <div className="space-y-3">
                            <Label htmlFor={`project-link-${index}`} className="text-gray-700">Project URL</Label>
                            <Input
                  ...
                  <TabsContent value="certs" className="p-6 space-y-6">
                    {(userInfo.certifications && userInfo.certifications.length > 0 ? userInfo.certifications : [{ name: "", issuingOrganization: "", dateObtained: "" }]).map((certification, index) => (
                      <Card key={index} className="p-0 border border-gray-200 shadow-sm relative overflow-hidden">
                        <div className="absolute top-0 left-0 w-1 h-full bg-blue-500"></div>
                        <CardHeader className="p-4 bg-gray-50">
                          <div className="flex justify-between items-center">
                            ...
                            {(userInfo.certifications?.length || 0) > 1 && (
                              <Button
                                variant="ghost"
                                size="sm"
                                className="text-red-500 hover:text-red-700 hover:bg-red-50"
                                onClick={() => removeCertificationBlock(index)}
                              >
                                <Trash2 size={14} className="mr-1" /> Remove
                              </Button>
                            )}
                        ...
                        <CardContent className="p-4 space-y-4">
                          <div className="space-y-3">
                            <Label htmlFor={`cert-name-${index}`} className="text-gray-700">Certification Name</Label>
                            <Input
                              id={`cert-name-${index}`}
                          ...
                          <div className="space-y-3">
                            <Label htmlFor={`cert-org-${index}`} className="text-gray-700">Issuing Organization</Label>
                            <Input
                              id={`cert-org-${index}`}
                          ...
                          <div className="space-y-3">
                            <Label htmlFor={`cert-date-${index}`} className="text-gray-700">Date Obtained</Label>
                            <Input
                              id={`cert-date-${index}`}
                              value={certification.dateObtained || ''}
                              onChange={(e) => handleInputChange('certifications', 'dateObtained', e.target.value, index)}
                    ...
                    <div className="pt-4 flex justify-between">
                      ...
                      >
                        {isGenerating ? (
                          <>
                            <CircleDashed className="mr-1 h-4 w-4 animate-spin" /> Generating...
                          </>
                        ) : (
                          <>
                            Generate Resume <RefreshCw className="ml-1 h-4 w-4" />
                          </>
                        )}
                      </Button>
                    </div>
