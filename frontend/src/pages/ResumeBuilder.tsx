...
const ResumeBuilder = () => {
  ...
  const printPreview = async (format: 'pdf' | 'docx' = 'pdf') => {
    const contentElement = previewRef.current;
    if (format === 'pdf' && !contentElement) {
    ...
    if (format === 'pdf') {
      // Temporarily remove shadow and fixed min-height for clean PDF render
      if (printPage) {
      ...
      // Inline stylesheets to ensure backend Puppeteer renders correctly
      // We grab all style tags from document head
    ...
    toast.info(`Generating ${format.toUpperCase()}... this may take a few seconds.`, {
      duration: 3000,
    });

    try {
      const response = await apiClient.post("/api/builder/export", {
        format,
        html,
        workspaceData: resume,
        title: resume.title
      ...
      link.href = url;
      link.setAttribute('download', `${(resume.title || 'Resume').replace(/\s+/g, '_')}.${format}`);
      ...
      <header className="flex h-12 flex-shrink-0 items-center justify-between border-b border-border/40 bg-secondary/30 backdrop-blur-md px-2 shadow-sm sm:h-14 sm:px-3 md:h-[60px] md:px-4 lg:h-[68px] lg:px-5">
        ...
        <div className="flex items-center gap-1 sm:gap-2">
          ...
          <Button
            variant="outline"
            size="sm"
            onClick={() => printPreview('docx')}
            className="hidden h-8 px-2 text-xs md:flex lg:px-3 lg:text-sm border-border/50 hover:bg-blue-500/10 hover:text-blue-600 transition-colors"
          >
            <Download className="h-3 w-3 lg:mr-1.5 lg:h-4 lg:w-4" />
            <span className="hidden lg:inline">Save as DOCX</span>
          </Button>
          <Button
