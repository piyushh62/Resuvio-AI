...
import { Download, Edit, FileText, MoreHorizontal, Plus, CircleDashed, AlertTriangle, UploadCloud, Bot, Copy, Trash2, PencilLine, Share2, Printer, BarChart3, Sparkles, Target, ScrollText, Zap, LayoutTemplate, Layers, FileEdit } from "lucide-react";
...
export default function MyDocuments() {
  ...
  const [isDeletingUploaded, setIsDeletingUploaded] = useState<string | null>(null);
  const [isDeletingCoverLetter, setIsDeletingCoverLetter] = useState<string | null>(null);
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      const results = await Promise.allSettled([
        ...
        apiClient.get('/api/cover-letter')
      ]);

      const [workspacesRes, uploadedRes, generatedRes, coverLettersRes] = results;

      if (workspacesRes.status === 'fulfilled' && workspacesRes.value.data && Array.isArray(workspacesRes.value.data.workspaces)) {
        setWorkspaces(workspacesRes.value.data.workspaces);
      } else {
        setWorkspaces([]);
        if (workspacesRes.status === 'rejected') console.error("Failed to load workspaces:", workspacesRes.reason);
      }

      if (uploadedRes.status === 'fulfilled' && uploadedRes.value.data && Array.isArray(uploadedRes.value.data.resumes)) {
        setUploadedResumes(uploadedRes.value.data.resumes);
      } else {
        setUploadedResumes([]);
        if (uploadedRes.status === 'rejected') console.error("Failed to load uploaded resumes:", uploadedRes.reason);
      }

      if (generatedRes.status === 'fulfilled' && generatedRes.value.data && Array.isArray(generatedRes.value.data.generatedResumes)) {
        setGeneratedResumes(generatedRes.value.data.generatedResumes);
      } else {
        setGeneratedResumes([]);
        if (generatedRes.status === 'rejected') console.error("Failed to load generated resumes:", generatedRes.reason);
      }

      if (coverLettersRes.status === 'fulfilled' && coverLettersRes.value.data && Array.isArray(coverLettersRes.value.data.coverLetters)) {
        setCoverLetters(coverLettersRes.value.data.coverLetters);
      } else {
        setCoverLetters([]);
        if (coverLettersRes.status === 'rejected') console.error("Failed to load cover letters:", coverLettersRes.reason);
      }
      
      // Check if all of them failed
      if (results.every(r => r.status === 'rejected')) {
         throw new Error("Failed to load any documents from the server.");
      }
  ...
  const handleOpenUploaded = async (id: string) => {
    navigate(`/dashboard/analyze?uploadedId=${id}`);
  };

  const handleDeleteUploaded = async (id: string) => {
    setIsDeletingUploaded(id);
    try {
      await apiClient.delete(`/api/resumes/${id}`);
      toast.success("Uploaded resume deleted");
      fetchData();
    } catch {
      toast.error("Failed to delete uploaded resume");
    } finally {
      setIsDeletingUploaded(null);
    }
  };

  const handleDeleteCoverLetter = async (id: string) => {
    setIsDeletingCoverLetter(id);
    try {
      ...
      <div className="max-w-full space-y-6 sm:space-y-8">
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          <div className="min-w-0">
            <h2 className="text-2xl font-bold sm:text-3xl">My Documents</h2>
            <p className="text-muted-foreground">Manage your resumes, drafts, and cover letters</p>
          </div>
  ...
  return (
    <div className="max-w-full space-y-4 sm:space-y-6">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between mb-2">
        <div className="min-w-0">
          <h2 className="text-3xl font-bold sm:text-4xl font-heading tracking-tight bg-gradient-to-r from-violet-600 to-blue-600 bg-clip-text text-transparent">My Documents</h2>
          <p className="text-muted-foreground mt-1.5 text-lg">Manage and organize your AI-crafted career documents</p>
        </div>
      </div>
        ...
        <div className="min-w-0 space-y-4 sm:space-y-6">
          ...
          <section>
            <div className="flex items-center justify-between mb-3">
              <h3 className="text-xl sm:text-2xl font-bold flex items-center gap-2 text-foreground font-heading mb-0">
                <div className="p-1.5 rounded-md bg-violet-500/10 text-violet-500 shrink-0">
                  <FileText className="h-5 w-5" />
                </div>
                Resume Drafts
              </h3>
              <Button onClick={handleCreateNew} size="sm" className="bg-violet-500/10 text-violet-700 hover:bg-violet-600 hover:text-white transition-all duration-300 rounded-lg shadow-sm hover:shadow-md hover:-translate-y-0.5 shrink-0 border-0 h-9 px-4 font-medium">
                <Plus className="h-4 w-4 mr-1.5" /> <span className="hidden sm:inline">Create Resume</span><span className="sm:hidden">Create</span>
              </Button>
            </div>
            <motion.div
              ...
              initial="hidden"
              animate="show"
              className="flex overflow-x-auto gap-4 pb-4 pt-1 snap-x snap-mandatory"
              style={{ scrollbarWidth: 'thin' }}
            >
              {/* Saved Workspace Cards */}
              {workspaces.length === 0 && (
                <p className="text-sm italic text-muted-foreground whitespace-nowrap px-2">No saved drafts yet.</p>
              )}
              ...
              {workspaces.map((workspace) => (
                <motion.div key={workspace.id} variants={cardVariant} className="w-[280px] sm:w-[320px] shrink-0 snap-start h-full">
                  <Card className="group overflow-hidden transition-all duration-300 hover:shadow-lg bg-card/60 backdrop-blur-md border-border/40 rounded-2xl hover:-translate-y-1 h-full flex flex-col">
                    <CardHeader className="bg-gradient-to-br from-violet-500/10 to-blue-500/10 border-b border-border/40 p-4 pb-3">
                    ...
                    <div className="mt-auto">
                      <CardFooter className="flex justify-end gap-3 border-t border-border/40 p-3 px-4 bg-secondary/20">
                        <Button
                          variant="ghost"
                          size="icon"
            ...
            <>
              ...
              <section>
                <div className="flex items-center justify-between mb-3">
                  ...
                  <Button onClick={() => navigate('/dashboard/analyze')} size="sm" className="bg-cyan-500/10 text-cyan-700 hover:bg-cyan-600 hover:text-white transition-all duration-300 rounded-lg shadow-sm hover:shadow-md hover:-translate-y-0.5 shrink-0 border-0 h-9 px-4 font-medium">
                    <Plus className="h-4 w-4 mr-1.5" /> <span className="hidden sm:inline">Analyze Resume</span><span className="sm:hidden">Analyze</span>
                  </Button>
                </div>
                <motion.div
                  variants={staggerContainer}
                ...
                >
                  {uploadedResumes.map((resume) => (
                    <motion.div key={resume.id} variants={cardVariant} className="w-[280px] sm:w-[320px] shrink-0 snap-start h-full">
                      <Card className="h-full flex flex-col overflow-hidden transition-all duration-300 hover:shadow-lg bg-card/60 backdrop-blur-md border-border/40 rounded-2xl hover:-translate-y-1">
                        <CardHeader className="bg-gradient-to-br from-cyan-500/10 to-blue-500/10 border-b border-border/40 p-4 pb-3">
                        ...
                        <div className="mt-auto">
                          <CardFooter className="flex justify-end border-t border-border/40 p-3 px-4 gap-2 bg-secondary/20">
                            <Button
                              variant="ghost"
                              size="sm"
                              className="h-9 w-9 p-0 text-muted-foreground hover:text-destructive hover:bg-destructive/10"
                              onClick={() => handleDeleteUploaded(resume.id)}
                              disabled={isDeletingUploaded === resume.id}
                              title="Delete Resume"
                            >
                              {isDeletingUploaded === resume.id ? <CircleDashed className="h-4 w-4 animate-spin" /> : <Trash2 className="h-4 w-4" />}
                            </Button>
            ...
            <>
              ...
              <section>
                <div className="flex items-center justify-between mb-3">
                  ...
                  <Button onClick={() => navigate('/dashboard/builder')} size="sm" className="bg-purple-500/10 text-purple-700 hover:bg-purple-600 hover:text-white transition-all duration-300 rounded-lg shadow-sm hover:shadow-md hover:-translate-y-0.5 shrink-0 border-0 h-9 px-4 font-medium">
                    <Plus className="h-4 w-4 mr-1.5" /> <span className="hidden sm:inline">AI Resume Builder</span><span className="sm:hidden">Build AI</span>
                  </Button>
                </div>
                <motion.div
                  variants={staggerContainer}
                ...
                >
                  {generatedResumes.map((resume) => (
                    <motion.div key={resume.id} variants={cardVariant} className="w-[280px] sm:w-[320px] shrink-0 snap-start h-full">
                      <Card className="h-full flex flex-col overflow-hidden transition-all duration-300 hover:shadow-lg bg-card/60 backdrop-blur-md border-border/40 rounded-2xl hover:-translate-y-1">
                        <CardHeader className="bg-gradient-to-br from-purple-500/10 to-pink-500/10 border-b border-border/40 p-4 pb-3">
                        ...
                        <div className="mt-auto">
                          <CardFooter className="flex justify-end border-t border-border/40 p-3 px-4 bg-secondary/20">
          ...
          <section>
            <div className="flex items-center justify-between mb-3">
              ...
              <Button onClick={() => navigate('/dashboard/cover-letter')} size="sm" className="bg-emerald-500/10 text-emerald-700 hover:bg-emerald-600 hover:text-white transition-all duration-300 rounded-lg shadow-sm hover:shadow-md hover:-translate-y-0.5 shrink-0 border-0 h-9 px-4 font-medium">
                <Plus className="h-4 w-4 mr-1.5" /> <span className="hidden sm:inline">Create Cover Letter</span><span className="sm:hidden">Create</span>
              </Button>
            </div>
            ...
            >
              ...
              {coverLetters.map((cl) => (
                  <motion.div key={cl.id} variants={cardVariant} className="w-[280px] sm:w-[320px] shrink-0 snap-start h-full">
                    <Card className="h-full flex flex-col overflow-hidden transition-all duration-300 hover:shadow-lg bg-card/60 backdrop-blur-md border-border/40 rounded-2xl hover:-translate-y-1">
                      <CardHeader className="bg-gradient-to-br from-emerald-500/10 to-teal-500/10 border-b border-border/40 p-4 pb-3">
                        <CardTitle className="truncate text-lg text-foreground font-heading">
                          Cover Letter
                        </CardTitle>
                        <p className="mt-1 text-sm font-semibold text-emerald-600 truncate">{cl.jobRole}</p>
                        <p className="truncate text-xs text-muted-foreground mt-0.5">{cl.companyName}</p>
                      </CardHeader>
                      <CardContent className="p-4 pt-2 flex-grow">
                        <div className="text-xs font-medium text-muted-foreground space-y-2 mb-3">
                          <p className="flex items-center"><CircleDashed className="mr-1.5 h-3 w-3" /> Saved: {formatDateString(cl.createdAt)}</p>
                        </div>
                        <p className="text-sm text-muted-foreground line-clamp-3 italic">"{cl.content.substring(0, 150)}..."</p>
                      </CardContent>
                      <div className="mt-auto">
                        <CardFooter className="flex justify-end gap-2 border-t border-border/40 p-3 px-4 bg-secondary/20">
                          <Button
                            variant="ghost"
                            size="sm"
                            className="h-9 w-9 p-0 text-muted-foreground hover:text-destructive hover:bg-destructive/10 mr-auto"
                            onClick={() => handleDeleteCoverLetter(cl.id)}
                            disabled={isDeletingCoverLetter === cl.id}
                            title="Delete Cover Letter"
                          >
                            {isDeletingCoverLetter === cl.id ? <CircleDashed className="h-4 w-4 animate-spin" /> : <Trash2 className="h-4 w-4" />}
                          </Button>
                          <Button variant="ghost" size="sm" className="h-9 font-medium hover:text-emerald-600 hover:bg-emerald-500/10" onClick={() => {
                              navigator.clipboard.writeText(cl.content);
                              toast.success("Copied to clipboard!");
                          }}>
                            <Copy className="mr-2 h-4 w-4" /> Copy
                          </Button>
                          <Button variant="outline" size="sm" className="h-9 font-medium border-border/50 hover:bg-emerald-500/10 hover:text-emerald-600 hover:border-emerald-500/30" onClick={() => navigate('/dashboard/cover-letter', { state: { coverLetter: cl } })}>
